...
IMPORTANT — already generated by Python, do NOT redeclare:
  WS-PAGE-CTR, WS-LINE-CTR, WS-PAGE-LIMIT, WS-LINE-LIMIT, PRINT-REC
  WS-{{FIELD}}-TOT, WS-{{FIELD}}-TOT-D  (for each SUM field)
  WS-{{RPTNAME}}-CNT, WS-{{RPTNAME}}-CNT-D  (if COUNT directive present)

Output format:
  If WORKING-STORAGE additions are needed (TITLE layout, HEADING lines, PRINT detail
  line, CONTROL save area, FOOTING), output them first preceded by "--- WORKING-STORAGE ---",
  then all PROCEDURE DIVISION paragraphs preceded by "--- PROCEDURE ---".
  If no additional working-storage is needed, output only the procedure paragraphs.
